...
-----------------------
Sauvegarde et transfère les paramètres Valorant (vidéo, crosshair, sensibilité,
keybinds...) entre plusieurs comptes Riot sur ce PC.
...
# Polices (affinées au démarrage selon ce qui est installé — voir _init_fonts)
FONT_UI = "Segoe UI"
...
def parse_video_settings(profile_path: Path) -> dict:
    ...
    if mode is not None:
        ...
        out["screen_mode"] = (modes_en if LANG == "en" else modes_fr).get(mode, mode)
    return out
...
class ChoiceDialog(_Dialog):
    ...
    def __init__(self, master, title: str, message: str, choices: list[tuple[str, str]],
        ...
        ctk.CTkLabel(self, text=message, font=(FONT_UI, 14, "bold"),
        ...
        for value, label in choices:
            ctk.CTkRadioButton(
                ...
                font=(FONT_UI, 13), text_color=C_TEXT,
        ...
        ctk.CTkButton(btns, text=confirm_text or T("Valider", "Confirm"), width=160,
                      fg_color=C_RED, hover_color=C_RED_HOVER,
                      font=(FONT_UI, 13, "bold"), command=self._ok).pack(side="right")
...
class TextDialog(_Dialog):
    ...
    def __init__(self, master, title: str, message: str, initial: str = ""):
        ...
        ctk.CTkLabel(self, text=message, font=(FONT_UI, 14, "bold"),
            ...
            padx=24, pady=(20, 10), anchor="w")
        self._entry = ctk.CTkEntry(self, width=400, height=36, font=(FONT_UI, 13),
        ...
        ctk.CTkButton(btns, text=T("Valider", "Confirm"), width=140, fg_color=C_RED,
                      hover_color=C_RED_HOVER, font=(FONT_UI, 13, "bold"),
...
class App(ctk.CTk):
    ...
    def _build_header(self):
        ...
        ctk.CTkLabel(left, text="VALORANT", font=(FONT_TITLE, 26, "bold"),
                     text_color=C_RED).pack(side="left")
        ctk.CTkLabel(left, text=" CONFIG MANAGER", font=(FONT_TITLE, 26),
                     text_color=C_TEXT).pack(side="left")

        right = ctk.CTkFrame(header, fg_color="transparent")
        right.pack(side="right", padx=20)
        ctk.CTkButton(right, text="EN" if LANG == "fr" else "FR", width=70, height=34,
        ...
        ctk.CTkButton(right, text=T(" Actualiser", " Refresh"), width=124, height=34,
                      ...
                      fg_color=C_CARD, hover_color=C_CARD_HOVER, text_color=C_TEXT,
                      border_width=1, border_color=C_BORDER,
                      font=(FONT_UI, 12), command=self.refresh).pack(side="right", padx=(10, 0))
    ...
    def _show_banner(self, riot_id: str):
        ...
        ctk.CTkLabel(self._banner, image=icon("user", 16, C_GREEN), compound="left",
                     ...
                     font=(FONT_UI, 13), text_color=C_TEXT).pack(side="left", padx=14)
    ...
    def _build_body(self):
        ...
        try:
            self.tabs._segmented_button.configure(font=(FONT_TITLE, 14))
        except Exception:
            pass
        ...
        ctk.CTkLabel(acc_panel, text=T("COMPTES DÉTECTÉS SUR CE PC", "ACCOUNTS FOUND ON THIS PC"),
                     ...
                     font=(FONT_TITLE, 16), text_color=C_TEXT).pack(
        ...
        ctk.CTkLabel(acc_panel,
                     ...
                     font=(FONT_UI, 11), text_color=C_TEXT_DIM).pack(anchor="w", padx=18)
        ...
        for label_fr, label_en, glyph, cmd in (
            ...
            ctk.CTkButton(actions, text=T(label_fr, label_en), height=28,
                          ...
                          fg_color=C_CARD, hover_color=C_CARD_HOVER, text_color=C_TEXT,
                          border_width=1, border_color=C_BORDER,
                          font=(FONT_UI, 11), command=cmd).pack(
    ...
    def _build_help(self, parent):
        ...
        def card(icon_name: str, title: str):
            ...
            ctk.CTkLabel(c, image=icon(icon_name, 17, C_RED), compound="left",
                         text="  " + title.upper(), font=(FONT_TITLE, 15),
        ...
        def text(parent_, s: str, dim: bool = False):
            ctk.CTkLabel(parent_, text=s, font=(FONT_UI, 12),
        ...
        def icon_row(parent_, icon_name: str, icon_color: str, name: str, desc: str):
            ...
            ctk.CTkLabel(row, text=name, font=(FONT_UI, 12, "bold"), text_color=C_TEXT,
                         width=200, anchor="w").pack(side="left")
            ctk.CTkLabel(row, text=desc, font=(FONT_UI, 12), text_color=C_TEXT_DIM,
        ...
        for col, (title, desc) in enumerate((
            ...
            ctk.CTkLabel(box, text=title, font=(FONT_TITLE, 13),
                         text_color=C_ORANGE).pack(anchor="w", padx=14, pady=(10, 0))
            ctk.CTkLabel(box, text=desc, font=(FONT_UI, 12), text_color=C_TEXT,
    ...
    def _build_statusbar(self):
        ...
        self.status = ctk.CTkLabel(
            bar, text=T("Prêt.", "Ready."), font=(FONT_UI, 11), text_color=C_TEXT_DIM)
        ...
        self.riot_status = ctk.CTkLabel(self.riot_pill,
                                        ...
                                        font=(FONT_UI, 11), text_color=C_TEXT_DIM)
    ...
    def _render_accounts(self):
        ...
        if not VALO_CONFIG_DIR.is_dir():
            ctk.CTkLabel(
                ...
                font=(FONT_UI, 13), text_color=C_ORANGE, justify="left").pack(
        ...
        if not self.accounts:
            ctk.CTkLabel(
                ...
                font=(FONT_UI, 13), text_color=C_TEXT_DIM, justify="left").pack(
                padx=14, pady=20, anchor="w")
            return
        ...
        for i, (folder, path, last) in enumerate(self.accounts):
            ...
            if self.connected_subject and folder.lower().startswith(self.connected_subject.lower()):
                ctk.CTkLabel(title_row, text=T("  ● CONNECTÉ", "  ● LOGGED IN"),
                             font=(FONT_UI, 9, "bold"), text_color=C_GREEN).pack(side="left")
            elif i == 0:
                ctk.CTkLabel(title_row, text=T("  DERNIER UTILISÉ", "  LAST USED"),
                             font=(FONT_UI, 9, "bold"), text_color=C_TEXT_DIM).pack(side="left")
            ...
            ctk.CTkLabel(info, text=T(f"Dernière activité : {fmt_date(last)}",
                         ...
                         font=(FONT_UI, 11), text_color=C_TEXT_DIM).pack(anchor="w")
    ...
    def _render_profiles(self):
        ...
        for prof in self.profiles:
            ...
            ctk.CTkLabel(name_row, text=prof["name"], font=(FONT_UI, 14, "bold"),
    ...
    def compare_profiles(self):
        ...
        for prof in (a, b):
            ctk.CTkLabel(header, text=prof["name"], font=(FONT_UI, 13, "bold"),
                         text_color=C_RED, width=210, anchor="w",
                         wraplength=200).pack(side="left", padx=4)

        summary_lbl = ctk.CTkLabel(
            dlg, font=(FONT_UI, 11), text_color=C_TEXT_DIM, anchor="w")
    ...
    def profile_details(self, prof: dict):
        ...
        lines.append((T("Créé le", "Created"), prof.get("created", "?").replace("T", " à " if LANG == "fr" else " at ")))
        ...
        dlg.transient(self)
        dlg.grab_set()
        ctk.CTkLabel(dlg, text=prof["name"], font=(FONT_UI, 16, "bold"),
        ...
        for label, value in lines:
            ...
            ctk.CTkLabel(row, text=value, font=(FONT_UI, 12), text_color=C_TEXT,
